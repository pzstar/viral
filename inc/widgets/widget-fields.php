...
function viral_pro_widgets_show_widget_field($instance = '', $widget_field = '', $viral_pro_field_value = '') {
    ...
    switch ($viral_pro_widgets_field_type) {
        ...
        case 'radio' :
            ...
            <p>
                <?php
                echo $viral_pro_widgets_title;
                echo '<br />';
                foreach ($viral_pro_widgets_field_options as $viral_pro_option_name => $viral_pro_option_title) {
                    ?>
                    <input id="<?php echo $instance->get_field_id($viral_pro_option_name); ?>" name="<?php echo $instance->get_field_name($viral_pro_widgets_name); ?>" type="radio" value="<?php echo $viral_pro_option_name; ?>" <?php checked($viral_pro_option_name, $viral_pro_field_value); ?> />
            ...
            <p>
                ...
                <select name="<?php echo $instance->get_field_name($viral_pro_widgets_name); ?>" id="<?php echo $instance->get_field_id($viral_pro_widgets_name); ?>" class="widefat">
                    <?php foreach ($viral_pro_widgets_field_options as $viral_pro_option_name => $viral_pro_option_title) { ?>
                        <option value="<?php echo esc_attr($viral_pro_option_name); ?>" id="<?php echo $instance->get_field_id($viral_pro_option_name); ?>" <?php selected($viral_pro_option_name, $viral_pro_field_value); ?>><?php echo esc_html($viral_pro_option_title); ?></option>
                    <?php } ?>
                </select>
        ...
        case 'number' :
            ?>
            <p>
                <label for="<?php echo $instance->get_field_id($viral_pro_widgets_name); ?>"><?php echo esc_html($viral_pro_widgets_title); ?>:</label><br />
                <input name="<?php echo $instance->get_field_name($viral_pro_widgets_name); ?>" type="number" step="1" min="1" id="<?php echo $instance->get_field_id($viral_pro_widgets_name); ?>" value="<?php echo absint($viral_pro_field_value); ?>" class="small-text" />
        ...
        case 'upload':
            ...
            <label for="<?php echo $instance->get_field_id($viral_pro_widgets_name); ?>"><?php echo esc_html($viral_pro_widgets_title); ?>:</label><br />
                ...
                <div class="placeholder<?php echo $image_class; ?>">
                    <?php _e('No image selected', 'viral-pro'); ?>
                </div>
                ...
                <div class="actions clearfix">
                    <button type="button" class="button viral-delete-button align-left"><?php _e('Remove', 'viral-pro'); ?></button>
                    <button type="button" class="button viral-upload-button alignright"><?php _e('Select Image', 'viral-pro'); ?></button>
...
function viral_pro_widgets_updated_field_value($widget_field, $new_field_value) {
    ...
    } elseif ($viral_pro_widgets_field_type == 'textarea') {
        // Check if field array specifed allowed tags
        if (!isset($viral_pro_widgets_allowed_tags)) {
            // If not, fallback to default tags
            $viral_pro_widgets_allowed_tags = '<p><strong><em><a>';
        }
        return strip_tags($new_field_value, $viral_pro_widgets_allowed_tags);
